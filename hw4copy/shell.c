...
int find_ampersand(command_t* p_cmd)
{
  int i = p_cmd->argc;
...
int find_gtr_symbol(command_t* p_cmd)
{
  int i = p_cmd->argc;
...
int execute(command_t* p_cmd)
{
  ...
  int cpid1= 0;
  int cpid2;
  int i;   // for debugging, remove later!
  pid_t c_pid = 0;  //process id
  int outfile;
  int child_process_status;


  if((gtr_symb = find_gtr_symbol(p_cmd)) > 0) {

    printf("*** FOUND < ***\n");

    outfile = open(p_cmd->argv[gtr_symb + 1], O_RDWR | O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP );
    ...
    if ( outfile == -1 ) {
      ...
      p_cmd->argv[gtr_symb] = NULL;
      dup2( outfile, 1 );
      execv("/bin/ls",p_cmd->argv);
      exit(-1);
    }
